...
export function App() {
  ...
  return (
    <PortraitContainer themeClass={appTheme === 'light' ? 'theme-light' : 'theme-dark'}>
      ...
      <main className={`flex-1 flex flex-col min-h-0 ${isLight ? 'bg-slate-50 text-slate-900' : 'bg-[#0b0f19] text-white'}`}>
        ...
        {currentMode === 'journey' && (
          <JourneyMode
            ...
            onFinishLesson={(count) => handleEarnPoints(50, true, count)}
        ...
        {currentMode === 'competition' && (
          <PassAndPlayMode
            ...
            onMatchFinished={(winnerIsUser) => handleEarnPoints(winnerIsUser ? 50 : 20, true, 0)}
            onEarnPoints={(pts, isMajor, count) => handleEarnPoints(pts, isMajor, count)}
          />
        )}
      ...
      {/* Google AdMob Banner Discreto nas Telas Principais (Início, Explorar, Progresso, Perfil) */}
      {/* Cumpre estritamente a Política para Famílias da Google Play: NUNCA exibido em provas, simulados ou resolução de exercícios */}
      {currentMode === 'tabs' && (
        <div className="fixed bottom-[58px] left-0 right-0 z-30 pointer-events-auto">
          <AdMobBanner
            currentMode={currentMode}
            theme={appTheme}
            onOpenAgeSettings={() => setIsAgeClassificationOpen(true)}
          />
        </div>
      )}
      ...
      {activeReminderToast && (
        <div className="fixed bottom-16 left-4 right-4 max-w-sm mx-auto bg-[#121829] text-white p-3 rounded-2xl shadow-xl flex items-center justify-between border border-blue-500/40 z-50 animate-in fade-in slide-in-from-bottom-2">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl overflow-hidden shadow-md shrink-0 border border-blue-400/40 bg-gradient-to-tr from-blue-900 to-indigo-700 flex items-center justify-center p-1">
              <img src="/icon.svg" alt="Trilha do Saber" className="w-full h-full object-contain" />
            </div>
            <div>
      ...
      {activeExamToast && (
        <div className="fixed inset-x-4 top-4 z-[999] max-w-md mx-auto p-4 rounded-3xl bg-[#0d121f] border-2 border-rose-500 shadow-2xl text-white animate-in slide-in-from-top-4 duration-300 ring-4 ring-rose-500/20">
          <div className="flex items-start gap-3">
            ...
            <div className="w-12 h-12 rounded-2xl overflow-hidden shadow-xl shrink-0 border-2 border-amber-400 bg-gradient-to-tr from-rose-950 via-amber-900 to-orange-700 p-1.5 flex items-center justify-center animate-pulse">
              <img src="/icon.svg" alt="Trilha do Saber" className="w-full h-full object-contain" />
            </div>
